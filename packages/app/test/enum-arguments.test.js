 ...
 * `edge_style = ` used to be answered with every global name in the language.
...
test('a fixed-set argument offers its values, not the whole language', () => {
  // The default first, so the list reads the way the signature above it does.
  assert.deepEqual(labels(completeAt('cylinder(h = 10, r = 4, edge_style = |);')), [
    '"chamfer"',
    '"round"',
  ]);
});
...
test('the values are offered once the quote is open, and filter as you type', () => {
  const result = completeAt('cylinder(h = 10, r = 4, edge_style = "ch|");');
  // Both are still offered — CodeMirror does the filtering, not this source.
  assert.deepEqual(labels(result), ['"chamfer"', '"round"']);
  // Replacing from the quote, so the completion cannot nest a second one.
  assert.equal(result.from, 'cylinder(h = 10, r = 4, edge_style = '.length);
});

test('a closing quote already in the document is not duplicated', () => {
  const result = completeAt('cylinder(h = 10, r = 4, edge_style = "|");');
  ...
  const open = completeAt('cylinder(h = 10, r = 4, edge_style = |);');
...
test('the parameter list is still offered where a parameter goes', () => {
  ...
  assert.ok(labels(result).includes('edge_style'), 'expected parameter names, not values');
});
...
test('a value the engine would reject is not in the catalogue', async () => {
  ...
  const engine = await Engine.create();
  const result = await engine.render('cylinder(h = 10, r = 4, chamfer = 1, edge_style = "bevel");');
  assert.ok(
    result.diagnostics.some((d) => d.severity === 'warning' && d.message.includes('edge_style')),
