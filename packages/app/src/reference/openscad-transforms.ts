  ...
  entries: [
    ...
    },
    {
      ...
      details: [
        ...
        'Which corners `r` rounds depends on the direction. Growing rounds the **outside** ' +
          'corners; shrinking rounds the **inside** ones. A convex shape has no inside corners, ' +
          'so `offset(r = -2) square(20)` is a smaller square with its corners still sharp — ' +
          'that is the right answer, not a missing round.',
        'Several children are offset as **one region**, not one at a time. Offsetting does not ' +
          'distribute over union, and the difference lands exactly on the edges the union makes: ' +
          'two overlapping squares form a cross, and shrinking it rounds the four inner corners ' +
          'that neither square had on its own.',
