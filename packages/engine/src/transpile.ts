...
const SHAPE_MODULES: Record<string, { params: string; body: string[] }> = {
  ...
  gear: {
    params:
      'm, teeth, h, internal = false, clearance = 0.2, pressure_angle = 20, helix = 0, ' +
      'center = false, segments = 0',
    body: [
      '// The involute of the base circle — the curve the end of a string traces',
      '// as it unwinds from it — is the flank, and is why two gears that share',
      '// a module and a pressure angle mesh without having been made together.',
      'function inv(a) = tan(a) * 180 / PI - a;',
      'function pol(x, a) = [x * cos(a), x * sin(a)];',
      'r = m * teeth / 2;',
      'rb = r * cos(pressure_angle);',
      '// The one number separating a gear from the solid a ring gear is cut',
      '// with: tip and root trade places, and the tooth takes the backlash the',
      '// external one gives up.',
      'grow = internal ? 1 : -1;',
      'rtip = internal ? r + 1.25 * m : r + m;',
      'rroot = internal ? r - m : r - 1.25 * m;',
      'half = 90 / teeth + grow * 45 * clearance / (PI * r);',
      'start = max(rb, rroot);',
      'function flank(x) = half + inv(pressure_angle) - inv(acos(min(1, rb / x)));',
      'tip_a = flank(rtip);',
      'root_a = flank(start);',
      'pitch_a = 360 / teeth;',
      '// The fragment count BetterSCAD derives, with the same floor: the',
      '// feature a gear has to resolve is the tooth, not the circle.',
      'auto = $fn > 0 ? max(3, floor($fn)) : max(5, ceil(min(360 / $fa, 2 * PI * rtip / $fs)));',
      'seg = max(4 * teeth, segments > 0 ? floor(segments) : auto);',
      'flank_steps = max(4, ceil(seg * (rtip - start) / (2 * PI * rtip)));',
      'tip_steps = max(1, ceil(2 * tip_a * seg / 360));',
      'root_steps = max(1, ceil((pitch_a - 2 * root_a) * seg / 360));',
      '// Negative is the right-hand helix, as it is on a thread.',
      'twist = -180 * h * tan(helix) / (PI * r);',
      'slices = max(1, ceil(abs(twist) * seg / 360));',
      'linear_extrude(height = h, center = center, twist = twist, slices = slices)',
      '  polygon([',
      '    for (k = [0 : teeth - 1])',
      '      let (c = k * pitch_a)',
      '        each concat(',
      '          // Below the base circle the string has run out and there is no',
      '          // involute left to draw, so the flank runs radially to the root.',
      '          rroot < rb ? [pol(rroot, c - root_a)] : [],',
      '          [for (i = [0 : flank_steps])',
      '            let (x = start + (rtip - start) * i / flank_steps) pol(x, c - flank(x))],',
      '          // The tip is an arc, not a chord: a chord planes it flat and',
      '          // makes the gear smaller than the diameter it claims.',
      '          tip_steps > 1',
      '            ? [for (i = [1 : tip_steps - 1])',
      '                pol(rtip, c - tip_a + 2 * tip_a * i / tip_steps)]',
      '            : [],',
      '          [for (i = [flank_steps : -1 : 0])',
      '            let (x = start + (rtip - start) * i / flank_steps) pol(x, c + flank(x))],',
      '          rroot < rb ? [pol(rroot, c + root_a)] : [],',
      '          root_steps > 1',
      '            ? [for (i = [1 : root_steps - 1])',
      '                pol(rroot, c + root_a + (pitch_a - 2 * root_a) * i / root_steps)]',
      '            : []',
      '        )',
      '  ]);',
    ],
